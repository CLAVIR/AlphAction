...
class InteractionBlock(nn.Module):
    def __init__(self, dim_person, dim_other, dim_out, dim_inner, structure_config,
        ...
        self.query = nn.Conv3d(dim_person, dim_inner, kernel_size=1, bias=bias)
        init_layer(self.query, init_std, bias)

        self.key = nn.Conv3d(dim_other, dim_inner, kernel_size=1, bias=bias)
        init_layer(self.key, init_std, bias)

        self.value = nn.Conv3d(dim_other, dim_inner, kernel_size=1, bias=bias)
        init_layer(self.value, init_std, bias)

        self.out = nn.Conv3d(dim_inner, dim_out, kernel_size=1, bias=bias)
        ...
        if dim_person != dim_out:
            self.shortcut = nn.Conv3d(dim_person, dim_out, kernel_size=1, bias=bias)
...
class IAStructure(nn.Module):
    def __init__(self, dim_person, dim_mem, dim_out, structure_cfg):
        ...
        self.person_dim_reduce = nn.Conv3d(dim_person, self.dim_inner, kernel_size=1, bias=bias)  # reduce person query
        init_layer(self.person_dim_reduce, conv_init_std, bias)
        self.reduce_dropout = nn.Dropout(structure_cfg.DROPOUT)

        if self.has_M:
            self.mem_dim_reduce = nn.Conv3d(dim_mem, self.dim_inner, kernel_size=1, bias=bias)
            init_layer(self.mem_dim_reduce, conv_init_std, bias)
        if self.has_P:
            self.person_key_dim_reduce = nn.Conv3d(dim_person, self.dim_inner, kernel_size=1,
                                                   bias=bias)  # reduce person key
            init_layer(self.person_key_dim_reduce, conv_init_std, bias)
        if self.has_O:
            self.object_dim_reduce = nn.Conv3d(dim_person, self.dim_inner, kernel_size=1, bias=bias)
