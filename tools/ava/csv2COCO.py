...
def my_csv2COCOJson(csv_path, movie_list, img_root, json_path, min_json_path):
    ann_df = pd.read_csv(csv_path, header=None)
    ...
    iter_num = len(ann_df)
    start_end_dict = {}
    for name in movie_ids:
        imgs = os.listdir(os.path.join(img_root, name))
        digital_lst = []
        for img in imgs:
            digital_lst.append(int(img.split('.')[0]))
        digital_lst.sort()
        start_end_dict[name] = [digital_lst[0], digital_lst[-1]]
    ...
    for rows in tqdm(ann_df.itertuples(), total=iter_num, desc='Calculating  info'):
        ...
        if movie_name not in movie_infos:
            ...
            width, height = movie_info['size']
            movie_id = movie_ids[movie_name] * 10000
            st = start_end_dict[movie_name][0]
            ed = start_end_dict[movie_name][1] + 1

            for tid in range(st, ed):
                img_id = movie_id + tid
                img_path = os.path.join(movie_name, '{}.jpg'.format(tid))
                video_path = os.path.join(movie_name, '{}.mp4'.format(tid))
                img_infos[tid] = {
...
def convert_csv2coco(video_name, title):
    from tools.ava.generate_csv2COCO_cmd import generate_cmd

    csv_path, movie_list, img_root = generate_cmd(video_name, title)
    json_path_temp = ""
    min_json_path_temp = ""
    ...
    if json_path_temp == "":
        if csv_path == "":
            ...
            dotpos = csv_path.rfind('.')
            if dotpos < 0:
                csv_name = csv_path
            else:
                csv_name = csv_path[:dotpos]
            json_path = csv_name + '.json'
    else:
        json_path = json_path_temp

    if min_json_path_temp == "":
        dotpos = json_path.rfind('.')
        if dotpos < 0:
            json_name = json_path
        else:
